...
export type ViewsByDayChartData = { date: string; views: number }[]
...
export function getViewsByDayChartData({
  ...
  return cacheFn({
    ...
    userId,
    interval,
  }) as Promise<ViewsByDayChartData>
}
...
async function getProductViewCountInternal({
  userId,
  startDate,
}: {
  userId: string
